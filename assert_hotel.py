...
def test_hotel():
    ...
    # Add room103 (needed for later tests) in addition to room101
    room103 = Room(103, "luxury", 300.0, 2)
    hotel = Hotel("Florida Beach", [room101, room103])
    ...
    try:
        ...
        assert str(
            e).strip() == "Room not available for the new selected dates", "Incorrect error message for overlapping update"
    ...
    # Active bookings:
    # - Booking 1 (Alice): 2025-01-16 to 2025-01-20 (4 days, 4*100 = 400)
    # - Booking 3 (Charlie): 2025-01-05 to 2025-01-09 (4 days, 4*100 = 400)
    total_income = hotel.get_total_income()
    expected_income_total = 8 * 100.0  # 8 days total at 100 per day = 800
    ...
    # Before cancellation, the 10-15 period should be unavailable (Eve's booking)
    assert room104.is_available_for(date(2025, 4, 10),
                                    date(2025, 4, 15)) == False, "Room 104 should be booked for first booking"
    hotel.cancel_booking(id4)
    # After cancellation, only Frank’s booking (starting at 15th) remains;
    # the 10-15 period should now be available.
    assert room104.is_available_for(date(2025, 4, 10),
                                    date(2025, 4, 15)) == True, "Room 104 should be available after cancellation"

    # 16. Additional test: ensure report reflects bookings accurately across rooms
    rep2 = hotel.report()
    # For room104, there should be 1 booking (id5)
    ...
    # With a unique counter, the new booking ID should equal hotel.next_booking_id - 1.
    assert id12 == hotel.next_booking_id - 1, "Booking ID consistency failed after cancellations"
    ...
    assert id14 == hotel.next_booking_id - 1, "Rebooking in the same room did not assign correct booking ID"
    ...
    # Room 103 should have 0 bookings
